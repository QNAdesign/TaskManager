...
import com.task.views.AddressOverlay;
	...
	protected void mapCurrentAddress() {
		...
		try {
			...
			if (null != addresses && addresses.size() > 0) {
				...
				AddressOverlay addressOverlay = new AddressOverlay(address); //decorate the map
			   mapOverlays.add(addressOverlay);
			   mapView.invalidate();
			   final MapController mapController = mapView.getController();
				mapController.animateTo(addressOverlay.getGeopoint(), new Runnable() {
					public void run() {
						mapController.setZoom(12);//zoom to the city level
					}
				});
				useLocationButton.setEnabled(true);
			} else {
				// Telling user no results for that address
			}
		} catch (IOException e) {
